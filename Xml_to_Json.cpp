...
int main(int argc, char* argv[]) {
    ...
    if (!root) {
        cout << "Error: Failed to parse XML" << endl;
        return 1;
    }
    ...
    if (!outputFile.empty()) {
        ...
        if (outFile.is_open()) {
            outFile << json;
            outFile.close();
            cout << "JSON successfully written to " << outputFile << endl;
        }
        else {
            cout << "Error: Cannot write to " << outputFile << endl;
            return 1;
        }
    }
    else {
